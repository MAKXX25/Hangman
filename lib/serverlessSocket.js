...
export class ServerlessSocket {
  ...
  emit(event, payload = {}, callback) {
    ...
    if (event === 'create_room') {
      ...
      this._handleJoin(payload, callback);
  ...
  // ── HOST: Create Room ─────────────────────────────────────────────────────
  _handleHostCreate({ playerName, wordPickTime, team = 'teamA', mode = '1v1' }) {
    this.isHost = true;
    ...
    this._trigger('room_created', { roomCode: this.roomCode, team: hostTeam, mode: roomMode });
    this._broadcastState();
    console.log(`[P2P Host] Created room ${this.roomCode} (${roomMode}) on ${hostTeam}`);
  }

  // ── JOINER: Join Room ─────────────────────────────────────────────────────
  _handleJoin({ roomCode, playerName, name, team = 'teamB' }, callback) {
    this.joinCallback = typeof callback === 'function' ? callback : null;
    ...
    if (!code || code.length < 4) {
      ...
      if (this.joinCallback) {
        this.joinCallback({ success: false, message: 'Please enter a valid room code.' });
      ...
      case 'request_join': {
        ...
        if (room.mode === '1v1') {
          ...
          if (totalPlayers >= 2) {
            this._sendToPeers({
              type: 'join_error',
              message: 'This 1v1 duel room is already full (2/2 players).',
            });
            return;
          }
          // In 1v1 duel: seat challenger on opposite team from host
          targetTeam = (room.teamA?.length || 0) === 0 ? 'teamA' : 'teamB';
          targetTeamName = targetTeam === 'teamA' ? 'Team A' : 'Team B';
        } else {
          targetTeam = msg.team === 'teamA' ? 'teamA' : 'teamB';
          targetTeamName = targetTeam === 'teamA' ? 'Team A' : 'Team B';
  ...
  _handleClientMessage(msg) {
    ...
    switch (msg.type) {
      case 'join_error':
        if (this.joinCallback) {
          this.joinCallback({ success: false, message: msg.message || 'Room is full or unavailable.' });
          this.joinCallback = null;
        }
      ...
      case 'state_update':
        if (this.joinCallback) {
          this.joinCallback({ success: true, room: msg.payload });
          this.joinCallback = null;
        }
