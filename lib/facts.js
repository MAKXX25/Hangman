...
/**
 * Shuffle Bag selector for Did You Know facts to prevent duplicates in a single session
 * @param {string[]} usedFacts List of fact strings already shown
 * @returns {string}
 */
export function getRandomFact(usedFacts = []) {
  const usedSet = new Set(usedFacts || []);
  let available = RANDOM_FACTS.filter(f => !usedSet.has(f));

  // If all facts have been shown, reset the shuffle bag
  if (available.length === 0) {
    available = RANDOM_FACTS;
  }
