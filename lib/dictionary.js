...
export async function validateAndFetchClue(word) {
  ...
  try {
    ...
    // If the response is a 404 (e.g., !res.ok), explicitly return null
    if (!res.ok) {
      return null;
    }
    ...
    // Use optional chaining to safely extract the definition
    const definition = data[0]?.meanings?.[0]?.definitions?.[0]?.definition;

    // If the fetch succeeds and definition exists, return the definition string
    if (definition) {
      return definition;
    }
    ...
    // If it throws an error in the catch block, explicitly return null
    return null;
  }
}
